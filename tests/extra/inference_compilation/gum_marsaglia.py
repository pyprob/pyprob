...
def produce_results(results_dir):
    ...
    for observe in observes:
        model_replace_true_is_posterior = model_replace_true.posterior_results(infer_traces, observe=observe)
        model_replace_false_is_posterior = model_replace_false.posterior_results(infer_traces, observe=observe)
    ...
    for j in range(train_traces_resolution):
        ...
        for i, observe in enumerate(observes):
            model_replace_true_ic_posterior = model_replace_true.posterior_results(infer_traces, observe=observe, inference_engine=InferenceEngine.IMPORTANCE_SAMPLING_WITH_INFERENCE_NETWORK)
            model_replace_false_ic_posterior = model_replace_false.posterior_results(infer_traces, observe=observe, inference_engine=InferenceEngine.IMPORTANCE_SAMPLING_WITH_INFERENCE_NETWORK)

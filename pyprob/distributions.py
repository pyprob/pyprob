...
class UniformContinuous(object):
    ...
    def sample(self):
        return np.random.uniform(self.prior_min, self.prior_max)

    def log_pdf(self, x):
        return scipy.stats.uniform.logpdf(x, self.prior_min, self.prior_max - self.prior_min)
...
class Flip(object):
    ...
    def __init__(self, proposal_probability):
    ...
    def sample(self):
        return 1 if np.random.uniform(0, 1) <= self.proposal_probability else 0
...
class MultivariateNormal(object):
    ...
    def set_proposal_params(self, tensor_of_proposal_mean_vars):
        num_dimensions = tensor_of_proposal_mean_vars.size(0) // 2

...
class Inference(object):
    ...
    def compile(self, default_observes=[], standardize_observes=False, directory='.', resume=False, lstm_dim=512, lstm_depth=2, obs_emb='fc', obs_reshape=None, obs_emb_dim=512, smp_emb_dim=32, one_hot_dim=64, softmax_boost=20, mixture_components=10, dropout=0.2,batch_size=64, valid_interval=1000, optimizer_method='adam', learning_rate=0.0001, momentum=0.9, weight_decay=0.0005, parallelize=False, truncate_backprop=-1, grad_clip=-1, max_traces=-1, keep_all_artifacts=False, replace_valid_batch=False, valid_size=256):
        # Initialize artifact
        ...
        if resume:
            ...
            traces = self.prior_traces(valid_size, default_observes)
    ...
    def prior_trace_nn_guided(self, *args, **kwargs):
        ...
        while True:
            ...
            yield trace
    def prior_traces_nn_guided(self, samples=1, *args, **kwargs):
        generator = self.prior_trace_nn_guided(*args, **kwargs)
        return [next(generator) for i in range(samples)]
    ...
    def posterior_samples(self, samples=10, *args, **kwargs):
        if self._artifact is None:
            ...
            traces = self.prior_traces_nn_guided(samples, *args, **kwargs)
            weights = np.array([trace.log_p for trace in traces])
